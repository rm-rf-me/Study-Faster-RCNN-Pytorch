...
class FasterRCNN(nn.Module):
    ...
    对应的三个阶段为特征提取、rpn和最终的类别和位置定位，详情见论文。
    ...
    Args:
        extractor (nn.Module): A module that takes a BCHW image
            ...
            vgg16时为除掉全链接层之前的所有部分，对应第一阶段即特征提取部分
        ...
        head (nn.Module): A module that takes
            ...
            对应第三阶段，根据原图片、RoI信息获得最终的分类和检测信息
    ...
    def use_preset(self, preset):
        ...
        该函数传入“visualize”或者“evaluate"来设置两种模式，两种模式中有不同的阈值参数，非极大值抑制的结果也不同
        可视化模式和评价模式
    ...
    def predict(self, imgs,sizes=None,visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            # 因为ProposalCreator对loc做了归一化（-mean /std）处理，
            # 所以这里需要再*std+mean，此时的位置参数loc为roi_cls_loc
            roi_cls_loc = (roi_cls_loc * std + mean)
            roi_cls_loc = roi_cls_loc.view(-1, self.n_class, 4)
            roi = roi.view(-1, 1, 4).expand_as(roi_cls_loc)
            cls_bbox = loc2bbox(at.tonumpy(roi).reshape((-1, 4)),   #将这128个roi利用roi_cls_loc进行微调，得到新的cls_bbox
            ...
            prob = at.tonumpy(F.softmax(at.totensor(roi_score), dim=1))     # 分类得分roi_scores经过softmax后转为概率prob

            raw_cls_bbox = at.tonumpy(cls_bbox)
            raw_prob = at.tonumpy(prob)
            # 此时得到的是对所有输入128个roi以及位置参数、得分的预处理，下面将筛选出最后最终的预测结果
