...
class RoI(Function):
    ...
    # 大小为[300, 512, 7, 7]。反正意思为将每个feature map 变成统一大小为7x7的。

    def __init__(self, outh, outw, spatial_scale):
        self.forward_fn = load_kernel('roi_forward', kernel_forward)    #用cupy编译后在gpu上跑
        self.backward_fn = load_kernel('roi_backward', kernel_backward) #具体的c代码在model/utils/roi_cupy.py中
    ...
    def forward(self, x, rois):
        ...
        output = t.zeros(N, C, self.outh, self.outw).cuda()         #初值全部为0
        self.argmax_data = t.zeros(N, C, self.outh, self.outw).int().cuda()
        self.rois = rois
        args = [x.data_ptr(), rois.data_ptr(),  #data_ptr()返回一个时间戳，numel()返回一个tensor变量内所有元素
                output.data_ptr(),
